...
import { TokenUsageByDay, TokenUsageByModel, TokenAnalytics } from './analytics.types'
...
export class AnalyticsService {
   ...
   */
  async getTokenAnalytics(api_key: string, days = 30): Promise<TokenAnalytics> {
    // Find the tenant
  ...
  private calculateTokenUsageByService(requests: any[]): Record<string, any> {
    ...
    Object.values(adapter_type).forEach((service) => {
