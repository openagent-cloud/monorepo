...
describe('ProxyController', () => {
  ...
  describe('streamProxyRequest', () => {
    it('should throw BadRequestException if no adapter is provided', async () => {
      ...
      await expect(controller.streamProxyRequest('', apiKey, dto, res)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should call service.handleStreamingRequest with correct parameters', async () => {
      ...
      expect(service.handleStreamingProxyRequest).toHaveBeenCalledWith(adapter, apiKey, dto.payload, res);
    });
  });
